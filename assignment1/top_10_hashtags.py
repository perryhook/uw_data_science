    ...
    for tweet in tweets:
        ...
        for word in set_tweet_words:
            if word not in word_scores:
                new_word_scores.setdefault(word, [])
                new_word_scores[word].append(score)
    # do averaging
    ...
    for tweet in tweets:
        ...
        if hashtags:
            for hashtag in hashtags:
                hashtag_cnt.setdefault(hashtag['text'], 0)
                hashtag_cnt[hashtag['text']] += 1
            # print "tweet {} with hashtags: {}".format(count, hashtags)
            # raw_input('press enter')
        count += 1
    top_hashtags = sorted(hashtag_cnt.items(), key=itemgetter(1), reverse=True)
    return top_hashtags[0:10]
...
def hw(sent_file, tweet_file):
    ...
    pprint(top_ten_hashtags(tweets))
